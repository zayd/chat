...
def load_params(path, params):
    pp = numpy.load("{0}.npz".format(path))
    for kk, vv in params.iteritems():
        if kk not in pp:
...
def evaluate(X, saveto, evaluate=False, out=False, theano_context=None):
    if theano_context is None:
      global theano
    else:
      theano = theano_context
    ...
    if evaluate:
        ...
        print "Image to text: {0}, {1}, {2}, {3}".format(r1, r5, r10, medr)
...
def trainer(train, dev, # training and development tuples
            ...
            dim_im=4800, # source sentence dimensionality
    ...
    for eidx in range(max_epochs):
        ...
        for minibatch in range(numbatches):
            ...
            cinds_im = conprng_im.randint(low=0, high=len(train[0]), size=ncon * len(im))
            cinds_s = conprng_s.randint(low=0, high=len(train[0]), size=ncon * len(s))
...
def rank(source, target, npts=None):
  if npts == None:
    npts = source.shape[0]
  ...
  for idx in range(npts):
    s = source[idx].reshape(1, source.shape[1])
    s /= norm(s)

    d = numpy.dot(s, target.T).flatten()
    index_list.append(numpy.argsort(d)[::-1])
